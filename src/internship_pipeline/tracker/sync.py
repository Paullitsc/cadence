...
Human discards are honored FIRST: any row whose Status the human set to
``rejected`` or ``withdrawn`` is recorded in storage (application status → that
same value, so no later sync resurrects it) and then deleted from the sheet. Only
after that is the upsert planned, against the post-deletion snapshot.
"""
...
from .rows import plan_answers_upsert, plan_applications_upsert, plan_status_removals
from .sheets import (
    ANSWERS_TAB,
    APPLICATIONS_TAB,
    apply_plan,
...
class SyncOutcome:
    ...
    rows_removed: int = 0  # human set Status to "rejected"/"withdrawn" → row deleted
...
def _process_removals(
    ...
    """Mark human-discarded rows (rejected/withdrawn) in storage, then delete
    them from the sheet.

    Storage first: if the deletion then fails, the next sync finds the row (its
    Status still reads "rejected"/"withdrawn") and retries; the reverse order
    could delete a row whose discard was never recorded, and the daily reconcile
    would re-append it. Returns the discarded dedupe keys.
    """
    row_numbers, removals = plan_status_removals(existing)
    if not row_numbers:
        return set()
    for key, status in removals:
        try:
            ...
            log.warning(
                "could not record removal in storage",
                extra={"key": key, "status": status, "error": repr(exc)},
            )
    delete_rows(services.sheets, spreadsheet_id, applications_sheet_id, row_numbers)
    log.info(
        "removed human-discarded rows from the sheet", extra={"rows": len(row_numbers)}
    )
    return {key for key, _ in removals}
...
def sync_applications_to_sheet(
    ...
    *,
    ...
    after honoring any Status the human set to ``rejected`` or ``withdrawn`` (row
    removed from the sheet, application marked with that status in ``storage``)."""
    ...
    removed_keys = _process_removals(
        services, spreadsheet_id, tab_ids[APPLICATIONS_TAB], existing, storage
    )
    if removed_keys:
        apps = [a for a in apps if a.dedupe_key not in removed_keys]
