...
# draft_outreach (application rows are final by then) and before log_and_digest
# (so the digest can link to the freshly-synced sheet). networking is
# job-independent and only needs to precede log_and_digest (the digest lists its
# due actions).
