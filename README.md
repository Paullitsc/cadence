...
helps (matching, résumé tailoring, outreach drafting).                                                                
...
what you want with the extras: `uv sync --extra phase2` (or `--extra ml` / `--extra llm` / `--extra render`).
...
## How orchestration ties together
...
## The Google Sheet is the application workspace; the inbox is for outreach
...
repo **variables**/**secrets** in Actions). **Everything is optional** — with none set the pipeline runs fully offline with deterministic fallbacks. Enable a capability by setting its vars. 
